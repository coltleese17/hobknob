Feature: General
     ...
     POST, PUT and PATCH requests should have non-empty payloads
     All POST, PUT and PATCH requests must have non-zero values for its
    ...
    All requests which has non-zero values for its "Content-Length"
    header must have its "Content-Type" header set
    ...
    All requests which has a "Content-Type" header must set its value to
